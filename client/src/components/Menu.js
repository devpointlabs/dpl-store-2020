...
const HomepageHeading = ({ mobile }) => <Container text></Container>;
...
const HomepageLayout = ({ auth }) => (
  <ResponsiveContainer auth={auth}></ResponsiveContainer>
);
...
export class ConnectedHomepageLayout extends React.Component {
  render() {
    return (
      <CartConsumer>
        {(auth) => <HomepageLayout {...this.props} auth={auth} />}
      </CartConsumer>
    );
  }
}
